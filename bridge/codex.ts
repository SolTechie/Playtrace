...
User request (data):\n${JSON.stringify(options.prompt)}`;
  ...
  try {
    ...
    if (options.kind === 'game' && result.game && !result.question && !result.game.sources.length)
      throw new Error('AI 未提供可核实的资料来源，请重试');
    return result;
  } finally {
