...
记录玩过的游戏，收藏走过的世界。支持手机和电脑浏览、管理游戏与查看统计。AI 资料整理由用户在自己的电脑上主动发起。
...
- 桌面 AI 指南：复制资料整理说明，在自己的电脑上主动使用 AI，检查资料后通过游戏表单保存。网页远程 AI 任务与设备配对已停用。

## 架构

React + Vite 构建响应式网页。Cloudflare Worker 提供 API，Supabase 提供 PostgreSQL 与 Storage。网页不派发 AI 任务，本机不运行任务轮询服务。

```text
浏览器 → Cloudflare Worker → Supabase

用户在电脑主动使用 AI → 检查资料 → 玩迹表单保存
```
...
远程 AI 接口在验证凭证与访问数据库之前被统一拒绝，旧浏览器或旧设备凭证不能绕过。数据库历史记录和权限未改变，部署回滚也必须保留该停用逻辑。独立桌面 API 令牌尚未提供。
