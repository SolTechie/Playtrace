...
远程 AI 接口在验证凭证与访问数据库之前被统一拒绝，旧浏览器或旧设备凭证不能绕过。数据库历史记录和权限未改变，部署回滚也必须保留该停用逻辑。Mac App 0.4.0 在系统原生授权窗口完成 Google 登录并自动返回，无需手动校验码。CLI 继续在系统浏览器授权；各自会话保存在 macOS 钥匙串。一次登录最多等待 5 分钟；没有常驻进程或远程 AI 任务。独立且可限定权限的 API 令牌尚未提供。
