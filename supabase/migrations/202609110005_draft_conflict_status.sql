-- A stale reviewed version is a business conflict, not a transient serialization failure.
-- PT409 returns immediately through PostgREST instead of retrying the transaction.
...
begin
  ...
  if job.kind='game' then
    if job.target_game_id is not null then
      ...
      if not found then raise exception using errcode='PT409', message='game changed or removed'; end if;
    else
