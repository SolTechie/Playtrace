...
确认管理员邮箱后，也可以执行 `node --env-file=.env.local scripts/create-admin.mjs 邮箱地址`。脚本仅供项目所有者使用：已有账号保持原密码，新账号的随机密码保存在被 Git 忽略的 `.env.admin`；不会发送邮件。

## 验证范围

自动检查覆盖输入、统计、PostgreSQL 权限、AI 任务租约与幂等保存、公开 API 配置和图片引用。真实 Codex 搜索可通过 smoke 脚本验证。可选 WebMCP 搜索接口已加入，实际注册和调用仍需在支持该协议的浏览器中验证；它不影响普通页面和 AI 工作台使用。
